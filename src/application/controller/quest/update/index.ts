...
/**
 * @typedef {object} UpdateQuestBody
 ...
 * @tags Quest
 * @security BearerAuth
 * @param {UpdateQuestBody} request.body
 ...
 */
export const updateQuestController: Controller =
  () =>
  async ({ lang, user, ...request }: Request, response: Response) => {
    try {
      ...
      await questRepository.update(
        { id: toNumber(request.params.id), createdById: user.id },
        { name, description }
      );
